"""ETH 15-minute four-shape strategy — provisional R3 live adapter.

This file is standalone and follows the interface of the user's existing
``eth_spike_r16_strategy.py``.  R3 is the provisional research combination:

    new F1 confirmation + F2 ANY + F3 bullish engulf + F4 balanced
...
def frozen_shape_configs() -> dict[str, ShapeConfig]:
    """Return the exact R3 signal registry used in the completed audit."""
    ...
    return {
        ...
        "F3": ShapeConfig(
            ...
            "CUSTOM_BULL_ENGULF", 4, 0.0, 0.75,
            "OPPOSITE_EXTREME_OR_TIMEOUT", "BULL_ENGULF", 0.30,
        ),
        "F4": ShapeConfig(
...
def load_position_config_from_env() -> RuntimePositionConfig:
    ...
    return RuntimePositionConfig(
        ...
        contract_step=_env_float("FOURSHAPE_CONTRACT_STEP", 1.0),
        min_contracts=_env_float("FOURSHAPE_MIN_CONTRACTS", 1.0),
        entry_api=entry_api,
    )
...
def new_next_open_entry_intents(bars: pd.DataFrame) -> list[NextOpenEntryIntent]:
    ...
    R3 uses this path for the provisional F3 bullish-engulf structure.
    Stop-confirmed F1/F2/F4 use ``new_pending_entry_intents``.  Multiple F3
    candidates confirming on the same bar are de-duplicated to one intent.
    """
...
class EthSpikeR3Strategy(StrategyBase):
    """R3 signal adapter; exact stop confirmations use the intent API."""
    ...
    @property
    def name(self) -> str:
        return "eth_spike_fourshape_r3_15m"
    ...
    def check_stop(self, current_position, entry_price, current_price, **kwargs) -> SignalResult:
        ...
        return SignalResult(
            ...
            "R3 TP/SL must be exchange-resident protective orders",
        )
        ...
        **kwargs,
        ...
        try:
            ...
            if len(df) < self.required_data_length:
                return SignalResult(SignalResult.NO_SIGNAL, f"R3 insufficient data: {len(df)}/128")
            ...
            if self.require_bar_closed_flag and kwargs.get("bar_is_closed") is not True:
                return SignalResult(SignalResult.NO_SIGNAL, "R3 blocked: closed-bar flag missing")
            bar_start = pd.Timestamp(df.index[-1])
            if self._last_processed_bar == bar_start:
                return SignalResult(SignalResult.NO_SIGNAL, "R3 duplicate closed bar blocked")
            ...
            if current_position != 0 and open_entries_raw is None:
                return SignalResult(SignalResult.NO_SIGNAL, "R3 blocked: open_entries required")
            ...
            if self.position_cfg.entry_api == "INTENTS":
                ...
                return SignalResult(
                    SignalResult.NO_SIGNAL,
                    f"R3 intent mode; stop_intents={stop_shapes}; next_open_intents={open_shapes}",
                )
            if raw_long and raw_short:
                return SignalResult(SignalResult.NO_SIGNAL, "R3 simultaneous long/short conflict rejected")
            if not raw_long and not raw_short:
                pending = new_pending_entry_intents(df)
                shapes = ",".join(i.shape for i in pending) or "-"
                return SignalResult(SignalResult.NO_SIGNAL, f"R3 no legacy signal; new_intents={shapes}")
            side = 1 if raw_long else -1
            if current_position == -side:
                return SignalResult(SignalResult.NO_SIGNAL, "R3 opposite signal rejected; no reversal")
            if current_position == side and open_entries >= self.position_cfg.max_same_direction_entries:
                return SignalResult(SignalResult.NO_SIGNAL, "R3 maximum same-direction entries reached")
            signal = SignalResult.LONG if side == 1 else SignalResult.SHORT
            reason = (
                f"variant=R3 shape={row.signal_shape} side={'LONG' if side == 1 else 'SHORT'} "
                f"reference={float(row.entry_reference_price):.4f} entry_api={self.position_cfg.entry_api}"
            )
            log_trade(f"[R3] {reason}")
            return SignalResult(signal, reason)
        except Exception as exc:
            logger.error(f"[R3] strategy error: {exc}", exc_info=True)
            return SignalResult(SignalResult.NO_SIGNAL, f"R3 fail-closed: {exc}")
